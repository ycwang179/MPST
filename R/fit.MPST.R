...
fit.MPST <- function(formula, lambda = NULL, method = NULL, P.func = NULL, data = list()) {
  ...
  # Fit the model
  mfit <- fit.mpst.internal(mpst.p$Y,mpst.p$Z, mpst.p$V, mpst.p$Tr, mpst.p$d, mpst.p$r, mpst.p$lambda, nl = 0, mpst.p$method, P.func = mpst.p$P.func)
...
#' @keywords internal
fit.mpst.internal <- function(Y, Z, V, Tr, d = NULL, r = 1, lambda, nl = 0, method, P.func) {
...
fit.mpst.d <- function(ind.Tr, Y, Z, V, Tr, d = NULL, r = 1, lambda, nl, load.all) {
  ...
  # ---- only special-case the single-simplex case ----
  if (d < 1 | r < 0) {
    warning("The degree of Bernstein polynomials d has be greater than zero, the smoothness parameter r has be nonnegative!")
    H <- NA; Q2 <- NA
  } else if (nrow(Trs) == 1) {
    # single triangle / tetrahedron: no cross-simplex smoothness constraints
    if (nd == 3) {
      nq.single <- (d + 2) * (d + 1) / 2
    } else if (nd == 4) {
      nq.single <- (d + 3) * (d + 2) * (d + 1) / 2 / 3
    }
    H <- matrix(0, nrow = 0, ncol = nq.single)
    Q2 <- diag(nq.single)
  } else {
    if (nd == 3) {
